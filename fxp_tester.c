...
int main(void) {
        ...
        for (unsigned int nfracbits=0; nfracbits <= 33; nfracbits++) {
                printf("\nTesting Frac bits   : %u\n", fxp_set_frac_bits(nfracbits));
                printf("Auto frac max dec is: %u\n", fxp_set_auto_frac_max_dec());
        }
        ...
        int nb = 1;
        do {
            printf("\n Number %u uses %d bits", n-1, fxp_nbits(n-1, FXP_INT_BITS));
            printf("\n Number %u uses %d bits", n, fxp_nbits(n, FXP_INT_BITS));
            n = (n << 1);
            nb++;
        } while (nb <= FXP_INT_BITS);
        printf("\n Number %u uses %d bits", UINT_MAX, fxp_nbits(UINT_MAX, FXP_INT_BITS));
        printf("\n");

        // Get # of frac bits from user
        int afb[] = {0, 12, 31};
        for (int fb = 0; fb < 3; fb++) {
            ...
            if (frac_bits > 0) {
                    ...
                    printf("\n");
                    for (int i = fxp_get_frac_max_dec() - 10; i <= fxp_get_frac_max_dec(); i++) {
                            printf("\nShowing fxp for 10.%3d: ", i);
                            print_fxp(fxp_dec(10, i));
                    }
                    printf("\n");
            ...
            printf("num is "); print_fxp( num); printf("\n");
            int fxp1    = num;
            int fxp2    = fxp(2);
            int nump2   = fxp_bin( whole + 2, bin_frac);    // a12
            int numm2   = fxp_bin( whole - 2, bin_frac);    // a8
            int mnumm2  = fxp_bin(-whole - 2, bin_frac);    // am12
            int mnump2  = fxp_bin(-whole + 2, bin_frac);   // am8
            int dnum    = fxp_bin( whole * 2, 2*bin_frac);  // a20
            int mdnum   = fxp_bin(-whole * 2, 2*bin_frac);  // am20
            int hnum    = fxp_bin( whole / 2, bin_frac/2);  // a5
            int mhnum   = fxp_bin(-whole / 2, bin_frac/2); // am5
            ...
            test_fxp("tiniest - 1",
                                fxp_sum(fxp_tiniest, fxp(-1)),
                                (frac_bits>0? fxp_bin(0, -frac_max): 0));
            test_fxp("-0.(+)500",
                                fxp_dec(-0, 500),
                                fxp_dec(0, 500));
            test_fxp("-0.(-)500",
            ...
            test_fxp("Max Fixed-Point number", FXP_MAX, FXP_POS_INF - 1);
            ...
            test_fxp("0.999 * 0.005", fxp_mul(fxp_bin(0,4095), fxp_bin(0, 24)), fxp_bin(0, 23));
            */
            printf("\nTotal # of warnings: %d\n", nwarnings);
